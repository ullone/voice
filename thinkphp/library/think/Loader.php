...
class Loader
{
    ...
    public static function autoload($class)
    {
        ...
        if (!empty(self::$namespaceAlias)) {
            $namespace = dirname($class);//返回路径中目录中的部分
            if (isset(self::$namespaceAlias[$namespace])) {
                $original = self::$namespaceAlias[$namespace] . '\\' . basename($class);//basename获取路径中文件名/类名
                if (class_exists($original)) {//根据类名判断类是否存在
                    return class_alias($original, $class, false);//为原有的类创建一个$class的别名
                }
            }
        }
    ...
    private static function findFile($class)
    {
        ...
        // var_dump('1'.$class);app\index\controller\Index
        // 查找 PSR-4
        $logicalPathPsr4 = strtr($class, '\\', DS) . EXT;//将$class中的\替换为DS
        // var_dump('2'.$logicalPathPsr4);app\index\controller\Index.php
        $first = $class[0];
        ...
        // 查找 PSR-0
        if (false !== $pos = strrpos($class, '\\')) {//查找'\'在$class中最后一次出现的位置
    ...
    public static function addNamespace($namespace, $path = '')
    {
        if (is_array($namespace)) {
            foreach ($namespace as $prefix => $paths) {
                self::addPsr4($prefix . '\\', rtrim($paths, DS), true);//移除paths右边的DS
            }
        } else {
